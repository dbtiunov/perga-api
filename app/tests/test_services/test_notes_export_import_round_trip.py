...
class TestNotesExportImportRoundTrip:
    def test_html_round_trip(self, test_db: Session, test_user):
        ...
        original_note = NoteService.create_note(
            test_db,
        ...
        content, filename = NotesExportService.export_single_note(
            test_db, user_id=test_user.id, note_id=original_note.id, export_type=ExportType.HTML
        )
        ...
        # ExportService adds <h1>Title</h1> to the body for HTML.
    ...
    def test_markdown_round_trip(self, test_db: Session, test_user):
        ...
        markdown_note = NoteService.create_note(
        ...
        content, filename = NotesExportService.export_single_note(
            test_db, user_id=test_user.id, note_id=markdown_note.id, export_type=ExportType.MARKDOWN
        )
    ...
    def test_zip_folder_recursive_round_trip(self, test_db: Session, test_user):
        ...
        NoteService.create_note(
            test_db,
            test_user.id,
            NoteCreateSchema(title='Note 2', body='<p>Body 2</p>', folder_id=child.id)
        )
    ...
    def test_duplicate_titles_round_trip(self, test_db: Session, test_user):
        # 1. Create two notes with the same title in same folder
        title = 'Duplicate'
